...
public class HextechSniperRifle extends RangedWeaponItem {
    ...
    public HextechSniperRifle(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        ...
        if (chargedProjectilesComponent != null && !chargedProjectilesComponent.isEmpty()) {
            this.shootAll(world, user, hand, itemStack, getSpeed(chargedProjectilesComponent), 1.0F, (LivingEntity)null);
    ...
    public void shootAll(World world, LivingEntity shooter, Hand hand, ItemStack stack, float speed, float divergence, @Nullable LivingEntity target) {
        if (world instanceof ServerWorld serverWorld) {
            ChargedProjectilesComponent chargedProjectilesComponent = (ChargedProjectilesComponent)stack.set(DataComponentTypes.CHARGED_PROJECTILES, ChargedProjectilesComponent.DEFAULT);
            if (chargedProjectilesComponent != null && !chargedProjectilesComponent.isEmpty()) {
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        if (!world.isClient) {
            ...
            if (f >= 0.2F && !this.charged) {
                this.charged = true;
                loadingSounds.start().ifPresent((sound) -> world.playSound((PlayerEntity)null, user.getX(), user.getY(), user.getZ(), (SoundEvent)sound.value(), SoundCategory.PLAYERS, 0.5F, 1.0F));
            }

            if (f >= 0.5F && !this.loaded) {
                this.loaded = true;
                loadingSounds.mid().ifPresent((sound) -> world.playSound((PlayerEntity)null, user.getX(), user.getY(), user.getZ(), (SoundEvent)sound.value(), SoundCategory.PLAYERS, 0.5F, 1.0F));
            }
        }
    ...
    public static record LoadingSounds(Optional<RegistryEntry<SoundEvent>> start, Optional<RegistryEntry<SoundEvent>> mid, Optional<RegistryEntry<SoundEvent>> end) {
        public static final Codec<LoadingSounds> CODEC = RecordCodecBuilder.create((instance) -> instance.group(
                SoundEvent.ENTRY_CODEC.optionalFieldOf("start").forGetter(LoadingSounds::start),
                SoundEvent.ENTRY_CODEC.optionalFieldOf("mid").forGetter(LoadingSounds::mid),
                SoundEvent.ENTRY_CODEC.optionalFieldOf("end").forGetter(LoadingSounds::end)
        ).apply(instance, LoadingSounds::new));

        public LoadingSounds(Optional<RegistryEntry<SoundEvent>> start, Optional<RegistryEntry<SoundEvent>> mid, Optional<RegistryEntry<SoundEvent>> end) {
            this.start = start;
            this.mid = mid;
            this.end = end;
        }
    ...
    public static enum ChargeType implements StringIdentifiable {
        ...
        public static final Codec<net.minecraft.item.CrossbowItem.ChargeType> CODEC = StringIdentifiable.createCodec(net.minecraft.item.CrossbowItem.ChargeType::values);
